...
-- Adds account_type field and weekly limit tracking to linkedin_accounts
-- ============================================================

-- 1. Add account_type and weekly limits to linkedin_accounts table
ALTER TABLE linkedin_accounts
  ADD COLUMN IF NOT EXISTS account_type TEXT NOT NULL DEFAULT 'free', -- free | premium | sales_navigator | recruiter
...
ALTER TABLE linkedin_accounts
  DROP CONSTRAINT IF EXISTS linkedin_accounts_account_type_check;
...
-- 4. Create function to reset weekly counters
...
COMMENT ON FUNCTION reset_weekly_counters IS 'Resets weekly action counters for all accounts at the start of each week (Monday)';
...
BEGIN
  ...
  -- If account not found, deny
  IF NOT FOUND THEN
    RETURN FALSE;
  END IF;

  -- Reset counters if week has changed
  ...
  -- Check if action would exceed weekly limit
...
-- 6. Update increment_action_count to also update weekly counters
...
BEGIN
  ...
  SET
    today_connections = CASE 
      WHEN p_action_type = 'connect' THEN today_connections + 1 
    ...
    this_week_connections = CASE 
      WHEN p_action_type = 'connect' THEN this_week_connections + 1 
      ELSE this_week_connections 
    END,
    this_week_messages = CASE 
      WHEN p_action_type = 'message' THEN this_week_messages + 1 
      ELSE this_week_messages 
    END,
...
COMMENT ON FUNCTION increment_action_count IS 'Increments action counters in daily_action_counts, weekly_action_counts, and linkedin_accounts tables after successful execution';
