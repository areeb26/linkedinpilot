...
import { useToggleAccount, useDeleteAccount, useUpdateAccount } from "@/hooks/useLinkedInAccounts"
import { MoreVertical, Settings, ShieldCheck, Flame, AlertCircle, ExternalLink, Trash2, RefreshCw } from "lucide-react"
...
export function AccountCard({ account }) {
  ...
  const sendProgress = (account.today_connections / account.daily_connection_limit) * 100
  const messageProgress = (account.today_messages / account.daily_message_limit) * 100
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-4">
            <div className="relative">
              ...
              />
              <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-[#1e1e1e] ${account.status === 'active' ? 'bg-green-500' : 'bg-gray-500'}`} />
            </div>
        ...
        <div className="grid grid-cols-2 gap-6 pt-2">
          <div className="space-y-2">
            <div className="flex justify-between items-end">
              ...
              <p className="text-[10px] font-bold text-white tracking-tighter">
                {account.today_connections}<span className="text-[#444]">/{account.daily_connection_limit}</span>
              </p>
            </div>
            <Progress value={sendProgress} className="h-1 bg-white/5" indicatorClassName="bg-blue-500" />
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-end">
              ...
              <p className="text-[10px] font-bold text-white tracking-tighter">
                {account.today_messages}<span className="text-[#444]">/{account.daily_message_limit}</span>
              </p>
            </div>
            <Progress value={messageProgress} className="h-1 bg-white/5" indicatorClassName="bg-purple-500" />
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-2 opacity-60 group-hover:opacity-100 transition-opacity">
          <Button 
            ...
            disabled={loading}
            className="flex-1 h-9 text-[10px] font-bold uppercase tracking-widest text-blue-400/50 hover:text-blue-400 hover:bg-white/5"
          >
            <RefreshCw className={loading ? "w-3.5 h-3.5 mr-2 animate-spin" : "w-3.5 h-3.5 mr-2"} />
            Sync
          </Button>
          <div className="w-px h-4 bg-white/5 self-center" />
          <Button variant="ghost" className="flex-1 h-9 text-[10px] font-bold uppercase tracking-widest text-[#444] hover:text-white hover:bg-white/5">
          ...
          <Button 
            ...
            className="flex-1 h-9 text-[10px] font-bold uppercase tracking-widest text-red-400/50 hover:text-red-400 hover:bg-white/5"
          >
